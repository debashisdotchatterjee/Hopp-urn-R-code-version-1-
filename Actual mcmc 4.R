...
model {
  alpha ~ gamma(2, 1);          // Prior on alpha (adjust as needed)
...
for (alpha in alpha_values) {
  ...
  fits[[paste0("alpha_", alpha)]] <- fit_model(counts, prior_alpha = 2)
}
...
plot_results <- function(fit, counts, alpha, base_distribution) {
  ...
  prior_dist <- data.frame(color = base_distribution, count = rep(prior_alpha, length(base_distribution)))
